...
void test_GetElementBoundary() {
	  // for constructor2
    int row=2,col=5;
    int data[2][5] = {{6,5,4,7,8},{9,8,6,5,4}};
    int*input_data[2];
    for (int i=0;i<2;i++){
    	input_data[i]=data[i];
    }
	...
	string actual ="Empty for Now";
	try {
			int output = f.getElement(-1, 0);	// invalid indexes
	}
	catch (std::out_of_range& e){
			 actual = e.what();
	}
...
void test_CopyBoundary() {

	int row=2,col=3;
    int data[2][3] = {{0,6,7}, {0,0,4}};
	int*input_data[2];
	for (int i=0;i<2;i++){
	    	input_data[i]=data[i];
	}
	Matrix origin =  Matrix(row,col,input_data);
    Matrix copied = origin.copy(); // make a copy
	string row1="0 0 0 \ n", row2="0 0 0 \n";
	string expected=row1+row2;
    ASSERT_EQUAL(expected,copied.toString());
}

void test_SetElementSame() {
	  int value = 1;
	  int setvalid_row = 0, setvalid_col = 0;
	  int row=2,col=5;
	      int data[2][5] = {{6,5,4,7,8},{9,8,6,5,4}};
...
void test_UpperBoundary() {

	int row=2,col=3;
    int upper1[2][3] = {{0,0,0},{0,0,0}}; // not a square matrix
	int*input_upper1[2];
	for (int i=0;i<2;i++){
	    	input_upper1[i]=upper1[i];
	}
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(test_GetElementBoundary));
	s.push_back(CUTE(test_CopyBoundary));
	s.push_back(CUTE(test_SetElementSame));
	s.push_back(CUTE(test_UpperBoundary));
